...
import { ShoppingCart, Trash2, Plus, Minus, FileText, ArrowRight, Package, Droplets, Leaf, Factory, LucideIcon } from "lucide-react"
    ...
    <Sheet>
      ...
      <SheetContent className="flex w-full flex-col sm:max-w-md bg-white p-0 gap-0 border-l shadow-2xl">
        ...
        <SheetHeader className="px-6 py-4 border-b border-slate-100 bg-slate-50/50">
          <SheetTitle className="text-lg font-semibold text-slate-900">Orden de Compra</SheetTitle>
          <p className="text-xs text-slate-500 font-medium">
            ID de Sesión: #{Math.floor(Math.random() * 10000).toString().padStart(4, '0')}
          </p>
        </SheetHeader>
          ...
          {cart.items.length === 0 ? (
            <div className="flex h-full flex-col items-center justify-center space-y-4 text-slate-400 p-6">
              ...
              <div className="text-center">
                <p className="font-medium text-slate-600">Orden vacía</p>
                      ...
                      <div className="flex justify-between items-start">
                        <div>
                          ...
                          <p className="text-xs text-slate-500 mt-0.5 font-mono">
                            {item.variant.name} • {formatPrice(item.variant.price)} un.
                          </p>
                        </div>
          ...
          <div className="bg-slate-50 border-t border-slate-200 p-6 space-y-4">
            <div className="space-y-1.5 text-sm">
              ...
              <div className="flex justify-between items-center">
                ...
                <span className="text-xl font-bold text-slate-900 tabular-nums">{formatPrice(total)}</span>
              </div>
            </div>
